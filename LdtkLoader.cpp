...
namespace {
	...
	/* Entity の GeneralId フィールド抽出 */
	int ParseEntityGeneralId(const JsonValue& entityValue) {
		const JsonValue* fieldInstancesValue = FindField(entityValue, "fieldInstances");
		if (fieldInstancesValue == nullptr || fieldInstancesValue->type != JsonType::Array) {
			return -1;
		}

		for (const JsonValue& fieldValue : fieldInstancesValue->arrayValue) {
			if (fieldValue.type != JsonType::Object) {
				continue;
			}

			const JsonValue* identifierValue = FindField(fieldValue, "__identifier");
			if (identifierValue == nullptr || identifierValue->type != JsonType::String) {
				continue;
			}

			if (identifierValue->stringValue != "GeneralId") {
				continue;
			}

			const JsonValue* valueValue = FindField(fieldValue, "__value");
			if (valueValue != nullptr && valueValue->type == JsonType::Number) {
				return AsInt(*valueValue, "GeneralId");
			}
		}
	...
	/* Entity の Count フィールド抽出 */
	int ParseEntityCount(const JsonValue& entityValue) {
		const JsonValue* fieldInstancesValue = FindField(entityValue, "fieldInstances");
		if (fieldInstancesValue == nullptr || fieldInstancesValue->type != JsonType::Array) {
			return -1;
		}
		...
		for (const JsonValue& fieldValue : fieldInstancesValue->arrayValue) {
			...
			if (identifierValue->stringValue != "Count") {
				continue;
			}

			const JsonValue* countValue = FindField(fieldValue, "__value");
			if (countValue != nullptr && countValue->type == JsonType::Number) {
				return AsInt(*countValue, "Count");
			}
		}
	...
	std::vector<LdtkEntityData> ParseEntities(const JsonValue& layerValue, int gridSize) {
		...
		for (const JsonValue& entityValue : entityArray) {
			...
			entity.generalId = ParseEntityGeneralId(entityValue);
			entity.count = ParseEntityCount(entityValue);
